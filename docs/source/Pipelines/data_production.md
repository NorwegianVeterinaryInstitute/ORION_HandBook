...
[section about paired-end
sequencing](../Seq_tech/sequencing_tech.md#Paired-end%20sequencing). This is
also known as Ukkonen’s condition, and for a deep dive into that, [please read
this
...
like _Escherichia coli_, where the known span of genome lengths vary with
...
region. This can happen in one of two main ways: either through _de novo_ gene
...
available genes are searched for in the genome. If prediction is done _de novo_,
...
motif, GC content bias, hexamer coding bias etc. _De novo_ gene finding tools
...
If _de novo_ methods have been used, this functional assignment is then a
...
section](../Typing/MLST.md)), the reads are commonly mapped to a database of
...
mapping, SNP calling (see [SNP calling section](../Clustering/snp_calling.md))
...
done (see [_de novo_ genome assembly section](#Assembly-and-annotation)), and
...
assembly, check [_de novo_ genome assembly section](#Assembly-and-annotation)),
