...
OA), for an overview of the methods that have been employed for L. monocytogenes
analyses).
...
- Pulsed Field Gel Electrophoresis (PFGE)  
  ...
  considered as the “gold-standard” for L. monocytogenes typing due to its high
  ...
  and discriminatory power in L. monocytogenes molecular typing until the advent
  of NGS technologies. It was thus being considered as the “gold-standard”
  typing method for L. monocytogenes for many years, and played an important
  role in L. monocytogenes surveillance and in the resolution of multiple
  outbreaks.
...
- MLVA (Multiple locus variable tandem repeat analysis)  
  ...
  microorganisms, but not L. monocytogenes. For this reason, it does not
  represent a standard method for surveillance of Listeria, but it is used by
...
- Multiplex-PCR for classifying 5 serogroups  
  ...
  serotyping discrimination by quickly classifying L. monocytogenes into 5
...
- MLST  
  ...
  7-gene MLST  (housekeeping genes) method was developed for L. monocytogenes
...
- Ribosomal multilocus typing (rMLST)  
...
- WGS   
  ...
  the new “gold-standard” in the analysis of L. monocytogenes ([Nadon et al.
  ...
  backward compatibility with previously mentioned molecular typing methods,as
  ...
  results have shown that using pWGS increases the number of outbreaks detected.
  Earlier outbreak detection facilitates timely action, thus facilitating
...
the capacity of the member states is already significant and progressing fast
for use of WGS as a complement or replacement technology for PFGE. 
...
clustering based on some distance, such as mash, ANI, allelic differences eg.
...
[Pightling et al.
2015](https://bmcresnotes.biomedcentral.com/articles/10.1186/s13104-015-1689-4)).
EGD-e NCI is the reference strain for Lineage II and F2365 for reference strain
for Lineage I ([Knudsen et al.
2017](https://sfamjournals.onlinelibrary.wiley.com/doi/abs/10.1111/1758-2229.12552)).
If theobjective is to discriminate between highly related samples that may/may
...
- Obtaining the de novo genome assembly of each sample, and:
  - Perform the respective genome annotation. Prokka is a commonly used program
    for L. monocytogenes genome annotation. BLAST or MUSCLE can be used to align
    ...
    monocytogenes genomes, is the use of a program like eggNOG mapper to perform
    functional annotation.
  - Use BLAST and MUSCLE to align a set of genes of interest on the genome
    assembly and identify the respective homologs.
- Some allele callers, such as chewBBACA, provide locus-specific alignments in
  an automated manner, being a good option to determine the allelic profile of
  samples.
...
of a cgMLST approach. In this context, there are two public cgMLST schemes which
...
Moura et al. (https://bigsdb.pasteur.fr/listeria/), while [Ridom
...
[https://www.cgmlst.org/ncs](https://www.cgmlst.org/ncs)). Besides cgMLST, some
studies also perform a MVLST analysis, considering a set of virulence-related
...
- [SNV-Phy](https://snvphyl.readthedocs.io/en/latest/)l (Canadian Public Health Agency)  
...
defined community, geographical area or season”. For foodborne disease,
outbreaks can be defined as two or more cases linked to the same food source
([Hoezler et al.
2018)](https://www.sciencedirect.com/science/article/abs/pii/S0740002017301739).
[Henri et al.
(2017)](https://www.frontiersin.org/articles/10.3389/fmicb.2017.02351/full)
showed that clustering of a diverse dataset of Listeria monocytogenes isolates
...
- ≤4, ≤7 cgMLST allelic differences (AD) considering any of the above-mentioned
  cgMLST schemes ([Van Walle et al.
  2018)](https://www.eurosurveillance.org/content/10.2807/1560-7917.ES.2018.23.33.1700798).
  [Cabal et al
  (2019)](https://www.frontiersin.org/articles/10.3389/fmicb.2019.02282/full),
  also considered clusters ≤ 10 AD, or
- Using PHE SnapperDB pipeline, [Nielsen et al.
  (2017)](https://efsa.onlinelibrary.wiley.com/doi/10.2903/sp.efsa.2017.EN-1151)
  ...
  outbreaks studied was between 8-21 HqSNPs. However, despite cutoff-thresholds
  ...
  are dependent on your workflow (ref). E.g., [Chen et al
...
case of infection or even an outbreak. As mentioned in the “Virulome and
